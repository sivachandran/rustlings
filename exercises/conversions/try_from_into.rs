...
impl TryFrom<(i16, i16, i16)> for Color {
    ...
    fn try_from(tuple: (i16, i16, i16)) -> Result<Self, Self::Error> {
        if tuple.0 < 0 || tuple.0 > 255 {
            return Err("tuple item 0 is outside boundry".to_string());
        }

        if tuple.1 < 0 || tuple.1 > 255 {
            return Err("tuple item 1 is outside boundry".to_string());
        }

        if tuple.2 < 0 || tuple.2 > 255 {
            return Err("tuple item 2 is outside boundry".to_string());
        }
    ...
    fn try_from(arr: [i16; 3]) -> Result<Self, Self::Error> {
        for i in 0..=2 {
            if arr[i] < 0 || arr[i] > 255 {
                return Err(format!("tuple item {} is outside boundry", i));
            }
        }
...
impl TryFrom<&[i16]> for Color {
    ...
    fn try_from(slice: &[i16]) -> Result<Self, Self::Error> {
        if slice.len() != 3 {
            return Err("slice doesn't have three elements".to_string());
        }
